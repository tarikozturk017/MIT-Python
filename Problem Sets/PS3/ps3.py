...
def get_word_score(word, n):
    ...
    # set component1 to 0
    comp1 = 0
    # itirate each letter to calculate values and store it in comp1
    for c in range(len(w)):
        comp1 += SCRABBLE_LETTER_VALUES[w[c]]
    # calculate comp2
    comp2 = 7*len(w) - 3*(n-len(w))
    if comp2 < 1:
        comp2 = 1
    score = comp1 * comp2
    return score
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if(is_valid_word(word, hand, word_list)):
                ...
                n = calculate_handlen(update_hand(hand, word))
